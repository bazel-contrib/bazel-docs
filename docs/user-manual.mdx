...
#### `--fdo_instrument=_profile-output-dir_`
...
`--fdo_instrument=_/path/to/rawprof/dir/_`.
...
#### `--java_language_version=_version_`
...
Possible values are: `local_jdk`, `local_jdk__version_`,
...
#### `--tool_java_runtime_version=_version_`
...
#### `--experimental_action_listener=_label_`
...
details from the [`action_listener`](/reference/be/extra-actions#action_listener) rule specified by _label_ to
...
#### `--experimental_extra_action_filter=_regex_`
...
#### `--android_platforms=_platform[,platform]*_`
...
#### `--per_file_copt=_[+-]regex[,[+-]regex]...@option[,option]..._`
...
#### `--custom_malloc=_malloc-library-target_`
...
#### `--java_toolchain=_label_`

No-op. Kept only for backwards compatibility.

#### `--host_java_toolchain=_label_`
...
#### `--strategy _mnemonic_=_strategy_`
...
#### `--strategy_regexp=_<filter,filter,...>=<strategy>_`
...
#### `--genrule_strategy=_strategy_`

This is a deprecated short-hand for `--strategy=Genrule=_strategy_`.
...
#### `--local_resources _resources or resource expression_`
...
`$TEST_SRCDIR/_canonical_repo_name_/_packagename_/_filename_`.
...
your [`--show_result _n_`](#show-result)
setting is high enough.

#### `--build_tag_filters=_tag[,tag]*_`
...
#### `--test_size_filters=_size[,size]*_`
...
#### `--test_timeout_filters=_timeout[,timeout]*_`
...
#### `--test_lang_filters=_string[,string]*_`
...
#### `--test_filter=_filter-expression_`
...
The particular interpretation of _filter-expression_ is up to
...
#### `--workspace_status_command=_program_`
...
#### `--toolchain_resolution_debug=_regex_`
...
#### `--flag_alias=_alias_name=target_path_`
...
#### `--default_visibility=_(private|public)_`
...
#### `--flaky_test_attempts=_attempts_`
...
#### `--test_timeout=_seconds_` OR `--test_timeout=_seconds_,_seconds_,_seconds_,_seconds_`
...
#### `--test_env=_variable_=_value_` OR `--test_env=_variable_`

Specifies additional variables that must be injected into the test
environment for each test. If _value_ is not specified it will be
...
By default, `bazel help [_topic_]` prints only a
...
In this case, `bazel info _key_` will print only
...
#### `--incremental_install_verbosity=_number_`
