...
requires = [
   ...
   <td><p>Feature-level. The feature is supported only if the specified required features are enabled. For example, when a feature is only supported in certain build modes (<code>opt</code>, <code>dbg</code>, or <code>fastbuild</code>). If <code>requires</code> contains multiple <code>feature_set</code>s the feature is supported if any of the <code>feature_set</code>s is satisfied (when all specified features are enabled).</p></td>
  </tr>
  <tr>
   <td><pre>
implies = ['feature']
