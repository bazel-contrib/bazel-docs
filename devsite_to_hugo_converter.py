...
class DevsiteToHugoConverter:
    ...
    def convert_documentation(self, source_path: str, output_path: str, 
                            dry_run: bool = False, incremental: bool = False) -> bool:
        """
        ...
        """
        try:
            logger.info(f"Starting conversion from {source_path} to {output_path}")
            ...
            devsite_structure = self.devsite_parser.parse_structure(source_path)
            ...
            conversion_stats = self._convert_content_files(
                devsite_structure, source_path, output_path, dry_run, incremental
            )
    ...
    def _convert_content_files(self, devsite_structure: Dict, source_path: str, 
                             output_path: str, dry_run: bool, incremental: bool) -> Dict:
        ...
        for md_file in source_dir.rglob('*.md'):
            ...
            try:
                ...
                if incremental and not self._file_needs_conversion(md_file, output_dir / 'content' / relative_path):
                    conversion_stats['skipped_files'] += 1
                    continue
                
                # Convert file
                if self._convert_single_file(md_file, output_dir / 'content' / relative_path, 
    ...
    def _convert_single_file(self, source_file: Path, output_file: Path, 
        ...
        try:
            ...
            hugo_frontmatter = self._convert_frontmatter(frontmatter, source_file, devsite_structure)
            
            # Extract title from H1 if not present in frontmatter
            if 'title' not in hugo_frontmatter:
                h1_match = re.search(r'^# (.+)$', body, re.MULTILINE)
                if h1_match:
                    hugo_frontmatter['title'] = h1_match.group(1).strip()
            ...
            if 'title' in hugo_frontmatter:
                hugo_body = self._remove_duplicate_h1_title(hugo_body, hugo_frontmatter['title'])
            
            # Generate Hugo markdown file
            hugo_content = self._generate_hugo_markdown(hugo_frontmatter, hugo_body)
    ...
    def _convert_body_content(self, body: str) -> str:
        ...
        # Convert [TOC] directive to Hugo shortcode
        body = re.sub(r'\[TOC\]', '{{< toc >}}', body)
        ...
        # Convert Devsite code blocks if needed
    ...
    def _remove_duplicate_h1_title(self, body: str, frontmatter_title: str) -> str:
        ...
        if first_line.startswith('# '):
            ...
            # If titles match, remove the H1 line and any following empty lines
            if h1_title == frontmatter_title:
                # Remove the H1 line
                remaining_lines = lines[1:]
                
                # Remove any empty lines immediately following the H1
    ...
    def _convert_internal_links(self, content: str) -> str:
        """
        Convert markdown links to Hugo relref shortcodes so they resolve correctly.
        ...
        -------
        str
            Markdown with `[text](path/to/file.md)` turned into
            `[text]({{< relref "path/to/file.md" >}})` (plus any anchor).
        """


        # Match [label](some/path/file.md#anchor)
        pattern = re.compile(r'\[([^\]]+)\]\((?!http)([^)]+?\.md)(#.*?)?\)')

        def _repl(m: re.Match) -> str:
            label = m.group(1)
            md_path = m.group(2)    # e.g. "guide/setup/config.md"
            anchor = m.group(3) or ''  # e.g. "#section"
            # Keep the .md in the shortcode argument
            return f'[{label}]({{{{< relref "{md_path}" >}}}}){anchor}'
    ...
    def _file_needs_conversion(self, source_file: Path, output_file: Path) -> bool:
    ...
    def _convert_assets(self, source_path: str, output_path: str) -> None:
        ...
        for css_file in css_files:
            self.css_converter.convert_css_file(css_file, output_dir / 'assets' / 'scss')
    ...
    def _generate_hugo_config(self, devsite_structure: Dict, output_path: str) -> None:
    ...
    def _generate_section_indices(self, devsite_structure: Dict, output_path: str) -> None:
        """Generate _index.md files for Hugo sections"""
        self.hugo_generator.generate_section_indices(devsite_structure, output_path)
