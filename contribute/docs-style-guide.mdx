...
Thank you for contributing to Bazel's documentation. This serves as a quick documentation style guide to get you started. For any style questions not answered by this guide, follow the [Google developer documentation style guide](https://developers.google.com/style).
...
- Make headers as short as is sensible. This way, they fit in the TOC without wrapping.
  ...
  - ✅ Yes: : Permissions
  - ⚠️ No: : A brief note on permissions

- Use sentence case for headings

  - ✅ Yes: : Set up your workspace
  - ⚠️ No: : Set Up Your Workspace
...
- Try to make headings task-based or actionable. If headings are conceptual, it may be based around understanding, but write to what the user does.
  ...
  - ⚠️ No: : On the preservation of graph order
...
- Capitalize proper nouns, such as Bazel and Starlark.
  ...
  - ⚠️ No: : At the end of the build, bazel prints the requested targets.

- Keep it consistent. Don't introduce new names for existing concepts. Where applicable, use the term defined in the [Glossary](/reference/glossary).
...
- Address your reader as "you". (If for some reason you can't use "you", use gender-neutral language, such as they.)
  ...
  - ⚠️ No: : For a user to build Java code with Bazel, he or she must install a JDK.
...
- Avoid "we". In user docs, there is no author; just tell people what's possible.
  ...
  - ⚠️ No: : Bazel is evolving, and we will make changes to Bazel that at times will be incompatible and require some changes from Bazel users.
...
- ⚠️ No: : Bazel will soon support remote caching, likely in October 2017.
...
- Use present tense. Avoid past or future tense unless absolutely necessary for clarity.
  ...
  - ⚠️ No: : If Bazel finds a dependency that does not conform to this rule, Bazel will issue an error.
...
- Where possible, use active voice (where a subject acts upon an object) not passive voice (where an object is acted upon by a subject). Generally, active voice makes sentences clearer because it shows who is responsible. If using active voice detracts from clarity, use passive voice.
  ...
  - ⚠️ No: : X is initiated by Bazel and then afterward Y will be built with the output.
...
- Avoid colloquial language. It's harder to translate phrases that are specific to English.
  ...
  - ✅ Yes: : Good rulesets
  - ⚠️ No: : So what is a good ruleset?
...
Note: Where possible, use Markdown instead of HTML in your files. Follow the [GitHub Markdown Syntax Guide](https://guides.github.com/features/mastering-markdown/#syntax) for recommended Markdown style.
...
- Use angle brackets to denote a variable that users should change. In Markdown, escape the angle brackets with a back slash: `\&lt;example\&gt;`.
  ...
  - ⚠️ No: : bazel help *command*: Prints help and options for "command"
