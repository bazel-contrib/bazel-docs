...
`--sandbox_backend=<a 'name=value' assignment>` multiple uses are accumulated
:   Registers a sandbox backend: a sandbox strategy implemented by an external binary that speaks Bazel's sandbox protocol. Value is &lt;name&gt;=&lt;binary&gt;, where &lt;name&gt; is the strategy name to select with --strategy and &lt;binary&gt; is an absolute path or a bare name resolved against PATH. May be repeated to register several backends. The binary is launched lazily as '&lt;binary&gt; [args...] serve' (see --sandbox_backend_opt). A backend reports available iff its binary exists and is executable; otherwise it declines per-spawn so Bazel falls through to the next strategy in --spawn_strategy.

    Tags:
    [`execution`](#effect_tag_EXECUTION)

`--sandbox_backend_opt=<a 'name=value' assignment>` multiple uses are accumulated
:   Appends one launch argument to a sandbox backend's server command line. Value is &lt;name&gt;=&lt;arg&gt;, matching a backend registered with --sandbox_backend. Repeat to pass multiple arguments; each occurrence contributes exactly one token (no quoting or splitting). Arguments are opaque to Bazel and forwarded verbatim to the backend binary via the Negotiate handshake (not as process arguments).
