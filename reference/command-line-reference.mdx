...
`--[no]incompatible_check_external_repo_source_dir_package_boundary` default: "false"
...
`--[no]incompatible_require_matching_aspect_hints_providers` default: "false"
...
`--[no]incompatible_symbolic_macro_strict_attrs` default: "false"
...
`--[no]incompatible_require_repo_extension_metadata` default: "false"
...
:   Specify a header in `NAME=VALUE` form that will be included in BES requests.
    ...
    Tags:
    [`affects_outputs`](#effect_tag_AFFECTS_OUTPUTS), [`fully_redacted_in_logs`](#metadata_tag_FULLY_REDACTED_IN_LOGS)
...
`--experimental_remote_cache_chunking_function=<auto, fast_cdc_2020 or rep_max_cdc>` default: "auto"
...
`--experimental_remote_circuit_breaker_recovery_delay=<An immutable length of time.>` default: "0"
...
`--experimental_remote_min_call_count_to_compute_failure_rate=<an integer>` default: "100"
...
`--experimental_remote_min_fail_count_to_compute_failure_rate=<an integer>` default: "12"
...
:   Specify a header that will be included in cache requests: --remote_cache_header=Name=Value. Multiple headers can be passed by specifying the flag multiple times. Multiple values for the same name will be converted to a comma-separated list.
    ...
    Tags:
    [`fully_redacted_in_logs`](#metadata_tag_FULLY_REDACTED_IN_LOGS)
...
`--remote_grpc_download_idle_timeout=<An immutable length of time.>` default: "60s"
...
`--remote_grpc_service_config=<a path>` default: see description
...
`--[no]grpc_tcp_keepalive` default: "true"
...
`--grpc_tcp_keepalive_count=<an integer>` default: "3"
...
`--grpc_tcp_keepalive_interval=<An immutable length of time.>` default: "10s"
...
`--grpc_tcp_keepalive_time=<An immutable length of time.>` default: "60s"
...
`--[no]experimental_split_coverage_postprocessing` default: "true"
...
`--flag_alias=<a 'name=label' flag alias>` multiple uses are accumulated
...
`--[no]experimental_fetch_all_coverage_outputs` default: "true"
...
`--[no]worker_sandboxing` multiple uses are accumulated
...
`--experimental_max_repeated_lost_inputs=<an integer>` default: "20"
...
`--[no]execution_log_binary_file` default: see description
...
`--[no]execution_log_compact_file` default: see description
...
`--[no]execution_log_json_file` default: see description
...
`--skycache_cache=<a string>` default: ""
:   Key-value store for dumping remote analysis cache entries from.

    Tags:
    [`bazel_monitoring`](#effect_tag_BAZEL_MONITORING)

`--skycache_fingerprint=<a string>` default: see description
:   Hex fingerprint of the Skycache entry to dump.

    Tags:
    [`bazel_monitoring`](#effect_tag_BAZEL_MONITORING)

`--skyframe=<off, summary, count, keys, value, deps, rdeps, function_graph, active_directories or active_directories_frontier_deps>` default: "off"
...
:   Deprecated no-effect flag. Only skylark mode is still supported.
    ...
    Tags:
    [`loading_and_analysis`](#effect_tag_LOADING_AND_ANALYSIS), [`execution`](#effect_tag_EXECUTION), [`incompatible_change`](#metadata_tag_INCOMPATIBLE_CHANGE), [`deprecated`](#metadata_tag_DEPRECATED)
...
`--[no]output:display_full_kind` default: "False"
...
| `fully_redacted_in_logs` | This option may contain sensitive credentials and should be redacted in logs. |
