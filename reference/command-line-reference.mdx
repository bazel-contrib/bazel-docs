...
:   Specifies the location of the repo contents cache, which contains fetched repo
    ...
    contents cache to be disabled, otherwise the default of `&lcub;--repository_cache&rcub;/contents`
    is used. Note that this means setting `--repository_cache=` would by default disable the
    repo contents cache as well, unless `--repo_contents_cache=&lcub;some_path&rcub;` is also set.
...
:   Specifies the cache location of the downloaded values obtained
    ...
    otherwise the default of `&lcub;--output_user_root&rcub;/cache/repos/v1` is used.

    Tags:
    [`bazel_internal_configuration`](#effect_tag_BAZEL_INTERNAL_CONFIGURATION)

`--[no]repository_disable_download` default: "false"
:   If set, downloading using `ctx.download&lcub;,_and_extract&rcub;` is not allowed during repository
...
:   Specified the module versions in the form of
    `&lcub;module1&rcub;@&lcub;version1&rcub;,module2@&lcub;version2&rcub;` that will be allowed in the resolved
...
:   Override a module with a local path in the form of `&lcub;module name&rcub;=&lcub;path&rcub;`. If the given
...
:   Specifies a list of notification keywords to be added the default set of keywords
    published to BES (`command_name=&lcub;command_name&rcub;`, `protocol_name=BEP`).
    Defaults to none.
...
:   Adds a new repository with a local path in the form of `&lcub;repository name&rcub;=&lcub;path&rcub;`. This
...
:   Override a repository with a local path in the form of `&lcub;repository name&rcub;=&lcub;path&rcub;`,
...
:   If enabled, the `&lcub;binary&rcub;.repo_mapping` file emits a module extension's repo mapping
...
:   The given features will be enabled or disabled by default for targets built in the target configuration.
    Specifying `-&lcub;feature&rcub;` will disable the feature. Negative features always override positive ones.
...
:   No-op. Kept here for backwards compatibility.
    ...
    Tags:
    [`eagerness_to_exit`](#effect_tag_EAGERNESS_TO_EXIT), [`incompatible_change`](#metadata_tag_INCOMPATIBLE_CHANGE), [`deprecated`](#metadata_tag_DEPRECATED)
...
:   Override the default resources amount for tests. The expected format is
    `&lcub;resource&rcub;=&lcub;value&rcub;`. If a single positive number is specified as `&lcub;value&rcub;`
    ...
    by `[-|*]&lcub;float&rcub;` (eg. `memory=HOST_RAM*.1,HOST_RAM*.2,HOST_RAM*.3,HOST_RAM*.4`).
...
:   Sets a shorthand name for a Starlark flag. It takes a single key-value pair in the form
    `&lcub;key&rcub;=&lcub;value&rcub;` as an argument.
...
:   Comma-separated list of aspects to be applied to top-level targets. In the list, if
    ...
    providers.
    `&lcub;bzl-file-label&rcub;%&lcub;aspect_name&rcub;`, for example `//tools:my_def.bzl%my_aspect`, where
