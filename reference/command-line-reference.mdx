...
`--[no]experimental_lazy_location_expansion` default: "false"
:   If set to true, ctx.expand_location accepts a lazy parameter that makes it return an opaque value for use with ctx.actions.args() instead of an eagerly expanded string.
