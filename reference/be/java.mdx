...
| --- | --- |
| `name` | [Name](/concepts/labels#target-names); required  A unique name for this target. |
| `deps` | List of [labels](/concepts/labels); default is `[]` The Java targets (including java_import and java_library) to collect transitive dependencies from. Runtime dependencies are collected via deps, exports, and runtime_deps. Resources are also collected. Native cc_library or java_wrap_cc dependencies are not. |
...
| `deploy_env` | List of [labels](/concepts/labels); default is `[]` A list of `java_binary` or `java_single_jar` targets which represent the deployment environment for this binary. Set this attribute when building a plugin which will be loaded by another `java_binary`. `deploy_env` dependencies are excluded from the jar built by this rule. |
...
| `output` | [Filename](/concepts/build-ref#filename); [nonconfigurable](common-definitions#configurable-attributes); default is `None` |
