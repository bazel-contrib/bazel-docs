...
- [alias](#alias)
- [config\_setting](#config-setting)
- [filegroup](#filegroup)
- [genquery](#genquery)
- [genrule](#genrule)
- [starlark\_doc\_extract](#starlark-doc-extract)
- [test\_suite](#test-suite)
...
- Tests are not run if their alias is mentioned on the command line. To define an alias
   that runs the referenced test, use a [`test_suite`](#test-suite)
   rule with a single target in its [`tests`](#test-suite.tests)
   attribute.
