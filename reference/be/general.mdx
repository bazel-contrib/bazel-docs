...
* If a flag takes multiple values (like `--copt=-Da --copt=-Db` or a list-typed
  [Starlark flag](https://bazel.build/rules/config#user-defined-build-settings)), `values = &lcub; "flag": "a" &rcub;` matches if `"a"` is
...
```
test_suite(name, aspect_hints, compatible_with, deprecation, features, licenses, package_metadata, restricted_to, tags, testonly, tests, visibility)
```
