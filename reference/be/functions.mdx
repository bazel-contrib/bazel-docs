...
Alias for [`default_package_metadata`](#package.default-package-metadata).
...
[exports\_files](#exports-files), which is public by default.
...
1. Since `glob()` runs during BUILD file evaluation,
    ...
    files to the glob. See the [example](#glob-example)
...
[expanded glob example](#expanded-glob-example) section below.
