...
| `default_deprecation` | String; default is `""`  Sets the default [`deprecation`](common-definitions.html#common.deprecation) message for all rules in this package. |
...
| `default_testonly` | Boolean; default is `False` except as noted  Sets the default [`testonly`](common-definitions.html#common.testonly) property for all rules in this package.  In packages under `javatests` the default value is `True`. |
...
In general, you should **try to provide an appropriate extension (e.g. \*.mdx)
