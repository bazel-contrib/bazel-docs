...
Dictionary: [label](/concepts/labels) -\> String; default is `{}`
...
Most \`@rules\_python//python/config\_setting\` settings can be used here, which
allows, for example, making only a certain \`py\_binary\` use
{obj}\`--boostrap\_impl=script\`.

Additional or custom config settings can be registered using the
{obj}\`add\_transition\_setting\` API. This allows, for example, forcing a
...
Support added for {obj}\`--bootstrap\_impl=system\_python\`.
:::
 `precompile`
