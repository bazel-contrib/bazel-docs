...
but please consider
[`additional_linker_inputs`](#cc_binary.additional_linker_inputs)
for that use case.
 `srcs`
...
Unlike `deps`, which is conceptually made for both compilation and
linking dependencies, `additional_linker_inputs` is specifically
made for only the latter, and signals a dependency that is required only for
linking (for example, files that are referenced in `linkopts`).
...
By default, C++ binaries are linked against `@bazel_tools//tools/cpp:link_extra_lib`,
which by default depends on the label flag `@bazel_tools//tools/cpp:link_extra_libs`.
...
but not to its dependents. Unlike `defines`, the defines are only added to the
