...
```
None export_repo(*args, module_name, dev_dependency=False)
```
...
without having to declare each one individually via `bazel_dep`.

This directive only takes effect in the root module; in other words, if a module
is used as a dependency by others, its own export\_repo directives are ignored.
...
After this, `@sensor_msgs` and `@rclcpp` are usable as if they
were declared with their own `bazel_dep` entries.

### Parameters

| Parameter | Description |
| --- | --- |
| `module_name` | [string](../core/string); required  The repo name of an existing `bazel_dep` whose transitive dependencies should be exported. |
| `dev_dependency` | [bool](../core/bool); default is `False`  If true, this export will be ignored if the current module is not the root module or `--ignore_dev_dependency` is enabled. |
| `args` | required  The names of the modules to export from the named module's dependencies. Each name will also be used as the local repo name. |
