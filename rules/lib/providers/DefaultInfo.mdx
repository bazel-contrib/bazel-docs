---
title: 'DefaultInfo {#modules.DefaultInfo}'
---
...
| `runfiles` | [runfiles](../builtins/runfiles); or `None`; default is `None`  [`runfiles`](../builtins/runfiles) descriptor describing the files that this target needs when run (e.g. via the `run` command or as a tool dependency for an action). |
...
```
File DefaultInfo.executable
```

A [`File`](../builtins/File) object representing the executable that should be executed to run the target, or `None` if unset. Should be set only for [`executable`](../globals/bzl#rule.executable) and [`test`](../globals/bzl#rule.test) targets.
May return `None`.
## files {#files}
