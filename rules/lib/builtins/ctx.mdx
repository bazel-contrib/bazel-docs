...
The result of this function is the same `Label` value as would be produced by passing the given string to a label-valued attribute of the rule and accessing the corresponding [Label()](../builtins/Target#label><code>label</code></a> field.
<p><i>Usage note:</i> The difference between this function and <a href=) is that `Label()` uses the context of the package of the `.bzl` file that called it, not the package of the target currently being analyzed. This function has the same behavior as [`native.package_relative_label()`](../toplevel/native#package_relative_label), which cannot be used in a rule or
...
| `expand_locations` | [bool](../core/bool); default is `False`  Shall we expand $(location) variables? See [ctx.expand\_location()](#expand_location) for more details. |
...
| `execution_requirements` | [dict](../core/dict); default is `{}`  Information for scheduling execution of command resolution. See [tags](/reference/be/common-definitions#common.tags) for useful keys. `execution_requirements` is not to be confused with the [`exec_properties`](/reference/be/common-definitions#common.exec_properties) rule attribute. Values will not be merged into the platform's execution properties, meaning they will only influnce spawn behavior. They will not be visible to remote executors. |
