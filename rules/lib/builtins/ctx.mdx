...
```
unknown ctx.expand_location(input, targets=[], *, lazy=False)
```
...
| `lazy` | [bool](../core/bool); default is `False`  **Experimental**. This parameter is experimental and may change at any time. Please do not depend on it. It may be enabled on an experimental basis by setting `--experimental_lazy_location_expansion`  Experimental: if true, returns an opaque value that can be passed to `Args.add` in place of the eagerly expanded string. Location paths are only rendered when the consuming action's command line is expanded, which retains less memory during analysis and makes the paths subject to path mapping. |
...
| `command` | [string](../core/string); required |
