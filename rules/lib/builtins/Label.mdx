...
- [same\_package\_label](#same-package-label)
- [workspace\_name](#workspace-name)
- [workspace\_root](#workspace-root)
...
**Deprecated.** This method behaves surprisingly when used with an argument containing an apparent repo name. Prefer [`Label.same_package_label()`](#same-package-label), [`native.package_relative_label()`](../toplevel/native.html#package_relative_label), or [`Label()`](#Label) instead.
...
**Deprecated.** The field name "workspace name" is a misnomer here; use the identically-behaving [`Label.repo_name`](#repo-name) instead.
