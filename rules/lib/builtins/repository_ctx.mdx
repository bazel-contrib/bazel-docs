---
title: 'repository_ctx {#modules.repository_ctx}'
---
...
```
None repository_ctx.extract(archive, output='', strip_prefix='', *, rename_files={}, watch_archive='auto', strip_components=0, type='')
```
...
```
None repository_ctx.patch(patch_file, strip=0, *, directory='', watch_patch='auto')
```

Apply a patch file to the root directory of external repository, or to `directory` within it. The patch file should be a standard  [unified diff format](https://en.wikipedia.org/wiki/Diff#Unified_format) file. The Bazel-native patch implementation doesn't support binary patch like the patch command line tool.
...
| `directory` | [string](../core/string); or [Label](../builtins/Label); or [path](../builtins/path); default is `''`  Directory to apply the patch in, relative to the repository directory. File names in the patch are resolved relative to this directory. Defaults to the repository directory. |
