...
Perform string interpolation. Format strings contain replacement fields surrounded by curly braces `&lcub;&rcub;`. Anything that is not contained in braces is considered literal text, which is copied unchanged to the output.If you need to include a brace character in the literal text, it can be escaped by doubling: `&lcub;&lcub;` and `&rcub;&rcub;`A replacement field can be either a name, a number, or empty. Values are converted to strings using the [str](../globals/all#str) function.
...
| `kwargs` | default is `&lcub;&rcub;`  Dictionary of arguments. |
...
| Parameter | Description |
| --- | --- |
| `elements` | iterable of [string](../core/string)s; required |
...
| --- | --- |
| `sep` | [string](../core/string); required  The string to split on. |
| `maxsplit` | [int](../core/int); default is `unbound`  The maximum number of splits. |
