...
```
 ...
 Do not use this field, its only purpose is to help with migration from config\_setting.values`{'compiler')` to config\_settings.flag\_values`{'@bazel_tools//tools/cpp:compiler'}`
