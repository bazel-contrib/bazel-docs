...
Returns a [dict](../core/dict.mdx) of environment variables that should be set for actions that need to run build tools on an Apple host system, such as the version of Xcode that should be used. The keys are variable names and the values are their corresponding values.
...
Creates a new [DottedVersion](../builtins/DottedVersion.mdx) instance.

### Parameters

| Parameter | Description |
| --- | --- |
| `version` | [string](../core/string.mdx); required  The string representation of the DottedVersion. |
