...
**Fork PRs and `docs2mdx.py` changes:** Reference docs (command-line reference, build encyclopedia, etc.) are generated by `docs2mdx.py`. For security, the pipeline does not run Bazel over arbitrary fork code. When a fork PR only changes `scripts/docs/**`, the preview workflow uses split-trust regen: HTML inputs are built from the trusted PR base commit, then the PR's `docs2mdx.py` runs via Python to produce fresh reference MDX. Fork PRs that change docgen inputs (`docgen/`, etc.) still cannot fully regen reference docs in preview.
